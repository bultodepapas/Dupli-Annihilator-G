import type { Dictionary } from "./en";

export const es: Dictionary = {
  "lang.name": "Español",
  "app.subtitle": "Panel de control de escritorio Tauri",
  "status.idle": "INACTIVO",
  "status.running": "EN EJECUCION",
  "status.done": "COMPLETADO",
  "status.error": "ERROR",
  "status.canceled": "CANCELADO",

  "section.inputs": "Entradas",
  "section.processing": "Procesamiento",
  "section.output": "Salida",
  "section.telemetry": "Telemetria",

  "button.add_files": "AGREGAR ARCHIVOS",
  "button.add_folder": "AGREGAR CARPETA",
  "button.pick_output": "ELEGIR SALIDA",
  "button.run": "EJECUTAR",
  "button.run_again": "EJECUTAR DE NUEVO",
  "button.retry": "REINTENTAR",
  "button.cancel": "CANCELAR",

  "field.inputs": "Archivos de entrada (una ruta absoluta por linea)",
  "field.output": "Ruta del archivo de salida",
  "field.mode": "Modo",
  "field.ordering": "Orden",
  "field.disk_alphabetical_mode": "Modo alfabetico en disco",
  "field.disk_buckets": "Buckets de disco",
  "field.disk_run_bytes": "Bytes por corrida en disco",
  "field.separator": "Separador",
  "field.separator_presets": "Preajustes de separador",
  "field.separator_preview": "Vista previa del separador",
  "field.raw_separator": "Separador literal (no interpretar escapes)",
  "field.allow_overwrite": "Permitir sobrescribir sin confirmacion",
  "field.language": "Idioma",
  "hint.drop_files": "Suelta archivos o carpetas aqui desde Explorer/Finder (TXT, CSV, PDF, EPUB…)",
  "tooltip.processing.mode":
    "Estrategia de ejecucion: auto elige por ti, ram es mas rapido, disk usa menos memoria.",
  "tooltip.processing.ordering": "Como se ordenan las lineas unicas en el archivo de salida.",
  "tooltip.processing.disk_alphabetical_mode":
    "Algoritmo alfabetico en disco: fast_bucket_local es mas rapido, global_perfect ordena globalmente de forma estricta.",
  "tooltip.processing.disk_buckets":
    "Cantidad de buckets en disco. Mas buckets reducen el uso de memoria por bucket.",
  "tooltip.processing.disk_run_bytes": "Tamano objetivo en bytes para cada corrida de ordenamiento en disco.",
  "tooltip.processing.trim": "Recorta espacios al inicio y final de cada linea antes de deduplicar.",
  "tooltip.processing.drop_empty": "Omite lineas vacias (despues de trim, si esta habilitado).",

  "option.mode.auto": "auto",
  "option.mode.ram": "ram",
  "option.mode.disk": "disk",
  "option.ordering.preserve_first_seen": "preserve_first_seen",
  "option.ordering.alphabetical": "alphabetical",
  "option.ordering.unordered_fast": "unordered_fast",
  "option.disk_mode.fast_bucket_local": "fast_bucket_local",
  "option.disk_mode.global_perfect": "global_perfect",

  "flag.trim": "trim",
  "flag.drop_empty": "drop_empty",
  "flag.drop_by_length": "drop_by_length",
  "flag.per_file_stats": "per_file_stats",
  "field.word_length_filter": "Filtro por longitud de palabra (chars)",
  "tooltip.processing.drop_length":
    "Omite tokens cuya longitud en caracteres esté entre [min..max] inclusive. Requiere ambos valores. Rango: 1–10.",
  "tooltip.processing.per_file_stats":
    "Recopila estadísticas por archivo: tokens, duplicados y tamaño por entrada. Solo modo RAM; el modo Disk devuelve null.",

  "meta.license": "Licenciado bajo PolyForm Small Business 1.0.0 — Licencia comercial requerida para grandes organizaciones",
  "meta.app": "app",
  "meta.backend": "backend",
  "meta.update_channel": "update_channel",
  "meta.job_id": "job_id",
  "meta.effective_separator": "separador_efectivo",

  "metric.stage": "etapa",
  "metric.files": "archivos",
  "metric.stage_items": "extrayendo",
  "metric.current_input": "archivo_actual",
  "metric.tokens": "tokens",
  "metric.unique": "unicos",
  "metric.duplicates": "duplicados",
  "metric.tps": "tps",
  "metric.elapsed": "elapsed",
  "metric.eta": "eta",
  "metric.preview_sample": "sample",
  "metric.elapsed_ms": "ms_transcurridos",
  "metric.eta_ms": "ms_estimados",
  "common.on": "ON",
  "common.off": "OFF",
  "running.panel_status": "Run status",
  "running.panel_config": "Frozen output config",
  "running.panel_actions": "Actions",
  "running.subline": "The workspace is locked while the active job writes and verifies results.",
  "running.narrative.extracting_text": "Reading source files and extracting tokens into the pipeline.",
  "running.narrative.tokenizing": "Scanning inputs and removing duplicates across the active corpus.",
  "running.narrative.sorting": "Ordering the unique result set for stable output.",
  "running.narrative.writing_output": "Writing the deduplicated output file to disk.",
  "running.narrative.finalizing": "Finalizing results and collecting end-of-run diagnostics.",
  "running.locked_hint": "These output settings are frozen for the current run.",
  "running.cancel_hint": "Cancel is the active control while the app is writing results.",
  "stage.extracting_text": "Extracting text",
  "stage.tokenizing": "Scanning and deduplicating",
  "stage.sorting": "Ordering unique results",
  "stage.writing_output": "Writing output",
  "stage.finalizing": "Finalizing run",

  "message.idle": "Inactivo",
  "message.job_started": "Trabajo iniciado",
  "message.done": "Completado",
  "message.canceled": "Cancelado",
  "message.error": "Error: {detail}",
  "message.cannot_start": "No se puede iniciar: {detail}",
  "message.starting": "Iniciando trabajo...",
  "message.start_failed": "Fallo al iniciar: {detail}",
  "message.cancel_requested": "Cancelacion solicitada",
  "message.cancel_failed": "Fallo al cancelar: {detail}",
  "message.preflight_failed": "Fallo en preflight: {detail}",
  "message.start_canceled_by_user": "Inicio cancelado por el usuario.",
  "message.input_dialog_failed": "Fallo en dialogo de entrada: {detail}",
  "message.folder_dialog_failed": "Fallo en dialogo de carpeta: {detail}",
  "message.output_dialog_failed": "Fallo en dialogo de salida: {detail}",

  "confirm.overwrite.title": "Confirmar sobrescritura",
  "confirm.overwrite.body": "El archivo de salida ya existe:\n{path}\n\nDeseas sobrescribirlo?",

  "validation.inputs_required": "Se requiere al menos un archivo de entrada.",
  "validation.output_required": "Se requiere la ruta del archivo de salida.",
  "validation.separator_required": "El separador no puede estar vacio.",
  "validation.disk_buckets_min": "Los buckets de disco deben ser >= 8.",
  "validation.disk_run_bytes_min": "Los bytes por corrida en disco deben ser >= 1,000,000.",

  "placeholder.inputs": "C:\\data\\in1.txt\\nC:\\data\\book.pdf",
  "placeholder.output": "C:\\data\\out.txt",
  "button.open_output": "OPEN OUTPUT",
  "button.open_folder": "OPEN FOLDER",
  "button.copy_report": "COPY REPORT",
  "button.export_json": "EXPORT JSON",
  "button.close_report": "CLOSE REPORT",
  "button.reset": "REINICIAR",
  "button.check_updates": "CHECK UPDATES",
  "button.checking_updates": "CHECKING...",
  "button.download_update": "DOWNLOAD UPDATE",
  "button.install_update": "INSTALL UPDATE",
  "button.installing_update": "INSTALLING...",
  "button.restart_to_apply": "RESTART TO APPLY",
  "summary.mission_report": "MISSION REPORT",
  "summary.title.success": "Run Complete",
  "summary.title.error": "Run Failed",
  "summary.title.canceled": "Run Canceled",
  "summary.subline": "Job {jobId} | {timestamp} | {mode} | {ordering}",
  "summary.section.key_results": "Key Results",
  "summary.section.output": "Output",
  "summary.section.performance": "Performance & Diagnostics",
  "summary.section.timeline": "Stage Timeline",
  "summary.section.diagnostics": "Quality & Safety",
  "summary.metric.unique_output": "Unique output",
  "summary.metric.total_scanned": "Total scanned",
  "summary.metric.duplicates_removed": "Duplicates removed",
  "summary.metric.reduction": "Reduction",
  "summary.metric.uniq_rate": "Uniq rate",
  "summary.metric.reduction_ratio": "Reduction ratio",
  "summary.metric.output_path": "Output path",
  "summary.metric.output_bytes": "Output bytes",
  "summary.metric.separator": "Separator",
  "summary.metric.elapsed": "Elapsed",
  "summary.metric.avg_tps": "Avg throughput (tok/s)",
  "summary.metric.peak_tps": "Peak throughput (tok/s)",
  "summary.metric.input_bytes": "Input bytes total",
  "summary.metric.mode_ordering": "Mode/Ordering",
  "summary.metric.normalization": "Normalization",
  "summary.metric.app_version": "App version",
  "summary.metric.backend_version": "Backend version",
  "summary.metric.update_channel": "Update channel",
  "summary.metric.disk_mode": "Disk alphabetical mode",
  "summary.no_stage_data": "No stage timing available.",
  "summary.no_warnings": "No warnings.",
  "summary.section.per_file": "Desglose por Archivo",
  "summary.per_file.file": "Archivo",
  "summary.per_file.size": "Tamaño",
  "summary.per_file.tokens_seen": "Tokens",
  "summary.per_file.duplicates": "Duplicados",
  "summary.per_file.unique_new": "Nuevos únicos",
  "summary.per_file.filtered": "Filtradas",
  "summary.per_file.not_collected": "No recopilado (activa per_file_stats o usa modo RAM).",
  "update.available_pill": "UPDATE {version}",
  "update.banner_title": "New version available: {version}",
  "update.banner_body": "Current version: {current}. Latest version: {latest}.",
  "update.major_notice": "Major release detected. Install manually from the release page.",
  "update.download_progress": "Downloading update: {pct}%",
  "message.summary_ready": "Summary ready.",
  "message.open_output_failed": "Open output failed: {detail}",
  "message.open_output_folder_failed": "Open output folder failed: {detail}",
  "message.report_copied": "Summary report copied.",
  "message.copy_report_failed": "Copy report failed: {detail}",
  "message.summary_exported": "Summary JSON exported.",
  "message.export_summary_failed": "Export summary failed: {detail}",
  "message.update_checking": "Checking for updates...",
  "message.update_available": "Update available: v{version}",
  "message.update_major_manual_only": "Major update v{version} requires manual install from Releases.",
  "message.update_up_to_date": "You are on the latest version: v{version}",
  "message.update_check_failed": "Update check failed: {detail}",
  "message.update_auto_unavailable": "In-app updater is unavailable for this release. Opening release page.",
  "message.update_install_blocked_running": "Finish or cancel the current job before installing the update.",
  "message.update_installing": "Installing update v{version}...",
  "message.update_ready_restart": "Update v{version} is ready. Restart to apply.",
  "message.update_install_failed": "Update install failed: {detail}",
  "message.update_restarting": "Restarting app to apply update...",
  "message.update_restart_failed": "Restart failed: {detail}",
  "message.open_release_failed": "Open release page failed: {detail}",
  "fallback.unknown_error": "unknown error",

  "section.checker": "Word Checker",
  "button.pick_file": "PICK FILE",
  "checker.path_placeholder": "Path to wordlist file...",
  "checker.load": "LOAD",
  "checker.loading": "LOADING...",
  "checker.ready": "Wordlist loaded: {count} words",
  "checker.loaded": "Loaded {count} words successfully.",
  "checker.load_failed": "Load failed: {detail}",
  "checker.word_placeholder": "Enter a word to check...",
  "checker.check": "CHECK",
  "checker.result_found": "FOUND",
  "checker.result_not_found": "NOT FOUND",
  "checker.check_failed": "Check failed: {detail}",

  "section.frequency": "Análisis de frecuencia",
  "freq.inputs_placeholder": "Archivos de entrada (una ruta por línea)...",
  "freq.top_n_label": "Top N",
  "freq.analyze": "ANALIZAR",
  "freq.analyzing": "ANALIZANDO...",
  "freq.result_summary": "{unique} únicos · {seen} total",
  "freq.col_rank": "#",
  "freq.col_token": "Token",
  "freq.col_count": "Cantidad",
  "freq.no_results": "Sin resultados.",
  "freq.error": "Análisis fallido: {detail}",

  "section.setop": "Operaciones de conjunto",
  "setop.left_label": "Izquierda (A)",
  "setop.right_label": "Derecha (B)",
  "setop.op_label": "Operación",
  "setop.output_label": "Archivo de salida",
  "setop.op.diff": "Diferencia  A − B",
  "setop.op.intersect": "Intersección  A ∩ B",
  "setop.op.union": "Unión  A ∪ B",
  "setop.run": "EJECUTAR",
  "setop.running": "EJECUTANDO...",
  "setop.result": "{unique} únicos escritos · {seen} tokens totales · {ms} ms",
  "setop.error": "Operación fallida: {detail}",
  "setop.inputs_placeholder": "Archivos de entrada (una ruta por línea)...",

  "section.fuzzy": "Agrupación difusa",
  "fuzzy.inputs_placeholder": "Archivos de entrada (una ruta por línea)...",
  "fuzzy.max_edit_label": "Distancia de edición máx.",
  "fuzzy.run": "EJECUTAR AGRUPACIÓN DIFUSA",
  "fuzzy.running": "EJECUTANDO...",
  "fuzzy.result": "{clusters} grupos · {exact} únicos exactos · {seen} escaneados · {ms} ms",
  "fuzzy.error": "La agrupación difusa falló: {detail}",
  "fuzzy.hint": "Agrupa tokens casi duplicados (p. ej. errores tipográficos) conservando un representante por grupo. O(u²) — mantenga los tokens únicos ≤ 100 000.",
};
